...
static void spake2__ge_precomp_0(
        spake2__ge_precomp_t *h) 
{
    spake2__fe_loose_1(&h->yplusx);
    spake2__fe_loose_1(&h->yminusx);
    spake2__fe_loose_0(&h->xy2d);
}

static void spake2__ge_precomp_cmov(
        spake2__ge_precomp_t *t, 
        const spake2__ge_precomp_t *u, 
        const uint64_t b) 
{
    spake2__fe_loose_cmov(&t->yplusx, &u->yplusx, b);
    spake2__fe_loose_cmov(&t->yminusx, &u->yminusx, b);
    spake2__fe_loose_cmov(&t->xy2d, &u->xy2d, b);
}
...
{
    spake2__fe_mul_tll(&r->X, &p->X, &p->T);
    spake2__fe_mul_tll(&r->Y, &p->Y, &p->Z);
    spake2__fe_mul_tll(&r->Z, &p->Z, &p->T);
}
// out1 = arg1
static void spake2__ge_p1p1_to_p3(
...
static void spake2__ge_madd(
        spake2__ge_p1p1_t *r, 
        const spake2__ge_p3_t *p, 
        const spake2__ge_precomp_t *q) {
  spake2__fe_t trY, trZ, trT;

  spake2__fe_add(&r->X, &p->Y, &p->X);
  spake2__fe_sub(&r->Y, &p->Y, &p->X);
  spake2__fe_mul_tll(&trZ, &r->X, &q->yplusx);
  spake2__fe_mul_tll(&trY, &r->Y, &q->yminusx);
  spake2__fe_mul_tlt(&trT, &q->xy2d, &p->T);
  spake2__fe_add(&r->T, &p->Z, &p->Z);
  spake2__fe_sub(&r->X, &trZ, &trY);
  spake2__fe_add(&r->Y, &trZ, &trY);
  spake2__fe_carry(&trZ, &r->T);
  spake2__fe_add(&r->Z, &trZ, &trT);
  spake2__fe_sub(&r->T, &trZ, &trT);
}
        ...
        const uint8_t precomp_table[15 * 2 * 32]) 
{
    ...
    // elements.
    spake2__ge_precomp_t multiples[15];
